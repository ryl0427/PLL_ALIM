...
def train(args, epoch, train_loader, model, loss_fn, loss_cont_fn, optimizer):
    ...
    cluster_piror_set_bingo_num = [0] * len(piror_set)
    ...
    for i, ((images_w1, images_s1, plabels, dlabels, index), (images_w2, _, _, _, index_2)) in enumerate(zip(train_loader, train_loader)):
        ...
        dlabels = dlabels.long().detach().cuda()

        # Train and save results
        classfy_out, cluster_out, cont_features, cont_labels = model(images_w1, images_s1, plabels, args)

        # Compute AUROC for the current batch
        dlabels_numpy = dlabels.detach().cpu().numpy()
        ...
        for jj in range(len(piror_set)):
            cluster_piror_set_bingo_num[jj] += torch.eq(torch.max(cluster_out * (plabels + piror_set[jj] * (1 - plabels)), 1)[1], dlabels).sum().cpu()
            classfy_piror_set_bingo_num[jj] += torch.eq(torch.max(classfy_out * (plabels + piror_set[jj] * (1 - plabels)), 1)[1], dlabels).sum().cpu()
        ...
        cont_mask = torch.eq(cont_labels[:batch_size], cont_labels.T).float().cuda()
        if epoch >= args.proto_start:
            if args.augmentation_type == 'case1':
                pred = cluster_out if args.proto_type == 'cluster' else classfy_out
        ...
        loss_cls = loss_fn(args, classfy_out, index)
