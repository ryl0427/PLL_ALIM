...
from pll_model_ood import PiCO
...
def train(args, epoch, train_loader,model, loss_fn, loss_cont_fn, optimizer):
    ...
    for  i,((images_w1, images_s1, plabels, dlabels, index),(images_w2,_,_,_, index_2)) in enumerate(zip(train_loader,train_loader)):
        ...
        classfy_out, cluster_out, cont_features, cont_labels, one_vs_all = model(images_w1, images_s1, plabels, args)
        ...
        positive_mask = plabels  # [batch_size, num_class]
        negative_mask = 1 - plabels
        one_vs_all_labels = positive_mask  # 正样本标签为1，负样本标签为0

        # 计算 one-vs-all 损失
        loss_one_vs_all = -(
            one_vs_all_labels * torch.log(torch.sigmoid(one_vs_all) + 1e-7)
            + negative_mask * torch.log(1 - torch.sigmoid(one_vs_all) + 1e-7)
        ).mean()
